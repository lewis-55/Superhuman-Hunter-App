...
const favouriteSuperheroPage = (() => {
    ...
    //---------------------------------------------
    // Function: Reners the Card Templates into the screen
    ...
    //---------------------------------------------
    // Function: Fetch Favourites super Heroes from the Local Storage
    ...
    if (!("favourites" in localStorage)) {
        ...
        // If the Favourites Array is Empty
        if (favourites.length === 0) {
            // Show the Message that there are no Favourites
            const superhero = template2.content.cloneNode(true).children[0];
            result.appendChild(superhero);
    ...
    //---------------------------------------------
    // Function: Handles the click function
  ...
  // If the superhero Card Image Clicked
  if (target.classList.contains("card-img-top")) {
    val = 
    target.nextElementSIbling.children[0].
         getAttribute("data-name");
       }
  // If the Superhero Card is Clicked
  if (target.classList.contains("card")) {
    val = target.querySelector(".card-title").
    getAttribute("data-name");
  }     
  // If the remove Button inside the Superhero Card is Clicked
   if (target.classList.contains("remove-btn")) {
    ...
    filter((favourite) => favourite !== name);
   ...
   // Set the Superhero Name (to Search for)
  ...
  //----------------------------------------------
  // Function: Initializes the Favourite SUperheroes
  const initializeApp = () => {
    // Clicked Event Listerner
    document.addEventListener("click",handleClick);
    // Runs on every Window Load/ Reload
    window.onload = () => {
 // Calls the fetchFavouritesSuperHeroes() function
